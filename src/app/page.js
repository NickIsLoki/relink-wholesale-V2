...
// MOCK DATA - material rollup with unit-level detail
...
const CATEGORIES = ['Patient Monitoring', 'Infusion', 'Imaging', 'Endoscopy', 'Surgical', 'Respiratory', 'Diagnostic'];
...
// Photo SVG - neutral product mocks on cream/sand background, brand-aligned

const photoSvg = (mfr, model, seed) => {

  const palettes = [

    ['#2E2622', '#4A3E3A'], ['#1f1814', '#3a3026'], ['#252019', '#3d342b'],

    ['#1a1612', '#2c241e'], ['#2a221c', '#473a32'], ['#221c17', '#382e26'],
  ...
  return `data:image/svg+xml;utf8,${encodeURIComponent(`
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 160">
      ...
      <defs>
        ...
        <radialGradient id="screen" cx="50%" cy="50%" r="50%">

          <stop offset="0%" stop-color="#0598A6" stop-opacity="0.45"/>
      ...
      <rect width="200" height="160" fill="#EFE9DA"/>

      <rect x="20" y="20" width="160" height="120" rx="6" fill="url(#g)"/>

      <rect x="32" y="32" width="136" height="80" rx="3" fill="#1a1310"/>

      <rect x="32" y="32" width="136" height="80" rx="3" fill="url(#screen)"/>

      <rect x="40" y="42" width="40" height="3" fill="#90AD51" opacity="0.85"/>

      <rect x="40" y="50" width="60" height="2" fill="#90AD51" opacity="0.5"/>

      <rect x="40" y="56" width="30" height="2" fill="#F38637" opacity="0.7"/>

      <circle cx="155" cy="45" r="3" fill="#0598A6" opacity="0.85"/>

      <circle cx="50" cy="125" r="2" fill="#6B615C"/>

      <circle cx="60" cy="125" r="2" fill="#6B615C"/>

      <text x="100" y="155" text-anchor="middle" font-family="'Source Sans 3', sans-serif" font-size="7" fill="#6B615C">${mfr.slice(0, 12)}</text>
...
const MATERIALS_DEF = [
  ...
  { oem: 'Olympus', material: 'OEV261H', commonName: '26" LCD Endoscopy Monitor', category: 'Endoscopy', basePrice: 4200, units: 8 },

  { oem: 'GE Healthcare', material: 'Carescape B450', commonName: 'Patient Monitor', category: 'Patient Monitoring', basePrice: 2400, units: 14 },

  { oem: 'Philips', material: 'IntelliVue MX450', commonName: 'Patient Monitor', category: 'Patient Monitoring', basePrice: 2850, units: 11 },

  { oem: 'CareFusion', material: 'Alaris PC Unit 8015', commonName: 'Infusion Pump Module', category: 'Infusion', basePrice: 850, units: 22 },

  { oem: 'Baxter', material: 'Sigma Spectrum', commonName: 'Infusion Pump', category: 'Infusion', basePrice: 1195, units: 18 },

  { oem: 'GE OEC', material: '9900 Elite', commonName: 'Mobile C-Arm', category: 'Imaging', basePrice: 38500, units: 4 },

  { oem: 'Puritan Bennett', material: '840 Ventilator', commonName: 'Ventilator', category: 'Respiratory', basePrice: 6200, units: 7 },

  { oem: 'Draeger Medical', material: 'Fabius GS Premium', commonName: 'Anesthesia Machine', category: 'Surgical', basePrice: 1575, units: 5 },

  { oem: 'Stryker', material: 'InTouch IM2131', commonName: 'CCU Bed', category: 'Surgical', basePrice: 8154, units: 6 },

  { oem: 'Hill-Rom', material: 'Progressa P7500', commonName: 'ICU Bed', category: 'Surgical', basePrice: 13254, units: 3 },

  { oem: 'Zoll', material: 'M Series', commonName: 'Defibrillator', category: 'Patient Monitoring', basePrice: 1850, units: 9 },

  { oem: 'Welch Allyn', material: 'Connex 6000', commonName: 'Vital Signs Monitor', category: 'Diagnostic', basePrice: 980, units: 16 },

  // Out-of-stock items in catalog - demo of restock-alert flow

  { oem: 'Mindray', material: 'BeneView T8', commonName: 'High-acuity Patient Monitor', category: 'Patient Monitoring', basePrice: 3200, units: 0 },

  { oem: 'Stryker', material: 'Stretcher 1105', commonName: 'Emergency Stretcher', category: 'Surgical', basePrice: 4800, units: 0 },

];

const ALL_UNITS = [];

const MATERIALS = MATERIALS_DEF.map((m, mi) => {

  const units = Array.from({ length: m.units }, (_, ui) => {

    const idx = mi * 100 + ui;

    const fn = FUNCTIONAL[idx % FUNCTIONAL.length];

    const cs = COSMETIC[(idx + 1) % COSMETIC.length];

    const hub = HUBS[idx % HUBS.length].id;

    const fnMult = { 'New': 1.0, 'Refurbished': 0.85, 'Tested Working': 0.7, 'Untested': 0.45, 'Non-Functional': 0.2 }[fn];

    const csMult = { 'New in Box': 1.05, 'Excellent': 1.0, 'Good': 0.95, 'Poor': 0.85 }[cs];

    const unit = {

      invSku: `Inv-${(637071 + idx).toString().padStart(8, '0')}`,

      serial: `${m.oem.slice(0, 3).toUpperCase()}${(900000 + idx).toString()}`,
      ...
      powersOn: fn !== 'Non-Functional' && fn !== 'Untested',

      unitPrice: Math.round(m.basePrice * fnMult * csMult),

      photo: photoSvg(m.oem, m.material, idx),

    };

    ALL_UNITS.push({ ...unit, materialKey: `${m.oem}::${m.material}` });
...
function WholesaleApp({ currentUser, users = [], tiers = [], onLogout, onOpenAdmin }) {
  ...
  const [expanded, setExpanded] = useState(new Set([MATERIALS[0].key]));
      ...
      <div style={{ width: '100%', maxWidth: 880, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'center' }}>
        ...
        <div className="hidden lg:block">
          ...
          <p style={{ fontSize: 14, color: C.stone, lineHeight: 1.6, maxWidth: 380, marginBottom: 28 }}>
            ...
            Search, build quotes, and submit orders across reLink's national inventory - 13,000+ devices, 6 hubs, real-time visibility into every unit.
          ...
          <div className="space-y-3">
            ...
            {[
              ...
              { icon: Search, text: 'Search 13,000+ devices monthly across 6 distribution hubs' },
